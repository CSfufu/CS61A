...
def quadratic(x, a, b, c):
    ...
    '0 to 10'
    """
    lower_value = a*lower_bound(x)**2+b*lower_bound(x)+c
    upper_value = a*upper_bound(x)**2+b*upper_bound(x)+c
    if a != 0:
        extreme_point = -b/(2*a)
        if a > 0:
            if extreme_point<=upper_bound(x) and extreme_point>=lower_bound(x):
                min_a = a*extreme_point**2+b*extreme_point+c
                max_a = max(lower_value,upper_value)
            else:
                min_a = min(lower_value, upper_value)   
                max_a = max(lower_value, upper_value)
        else:
            if extreme_point<=upper_bound(x) and extreme_point>=lower_bound(x):
                max_a = a*extreme_point**2+b*extreme_point+c
                min_a = min(lower_value,upper_value)
            else:
                min_a = min(lower_value, upper_value)   
                max_a = max(lower_value, upper_value)
