...
def pingpong(n):
    ...
    True
    """
    def ping_pong_helper(wave_number, res, step):
        if res == n:
            return wave_number
        if res % 8 == 0 or num_eights(res) > 0:
            return ping_pong_helper(wave_number-step,res+1,-step)
        else:
            return ping_pong_helper(wave_number+step,res+1,step)
    return ping_pong_helper(1,1,1)
"""
def fun(n):
    i = 1
    count = 0
