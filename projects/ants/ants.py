...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        distance = 0
        while search_place is not beehive:
            if len(search_place.bees) != 0 and self.min_range <= distance <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        now_location = self.place
        Ant.reduce_armor(self, amount)
        if self.armor <= 0:
            amount += self.damage
        bees = now_location.bees[:]
        for bee in bees:
...
class WallAnt(Ant):
    ...
    which is an ant that does nothing each turn. A WallAnt is useful because it has a large armor value.
    """
    name = 'Wall'
    food_cost = 4
    implemented = True
...
class ScubaThrower(ThrowerAnt):
    ...
    Implement the ScubaThrower, which is a subclass of ThrowerAnt that is more costly and watersafe, but otherwise identical to its base class. 
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        if self.imposter is True:
            ...
            while curr_place is not None:
                if curr_place.ant and curr_place.ant.buff is False:
                    curr_place.ant.damage = curr_place.ant.damage * 2
                    curr_place.ant.buff = True
                if isinstance(curr_place.ant, ContainerAnt) \
                   and curr_place.ant.contain_ant \
                   and curr_place.ant.contain_ant.buff:
                    curr_place.ant.damage = curr_place.ant.damage * 2
                    curr_place.ant.buff = True
    ...
    def reduce_armor(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        Ant.reduce_armor(self, amount)
        if self.armor <= 0 and self.imposter is False:
            bees_win()
        # END Problem EC
